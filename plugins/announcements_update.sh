#!/usr/bin/bash
# a script that downloads the latest announcements

# download URL
url="$1"
gamedir="$2"
# bypass modsecurity
usaragent="-d --user-agent=announcements-updater-script"
# parse the filename from the download path
filename=$(echo $url | sed -n 's/^\(.*\/\)*\(.*\)/\2/p')

# script to download latest announcements.txt from github
...
if [[ "$url" == *"githubusercontent"* ]]; then
  ...
  wget -O $filename $url 2>&1 | tee | grep -E 'Omitting download|`announcements.txt` saved|ERROR 404: Not Found'
else
  ...
  wget $usaragent -N $url 2>&1 | tee | grep -E 'Omitting download|`announcements.txt` saved|ERROR 404: Not Found'
fi
